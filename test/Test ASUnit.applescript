...
script |ASUnit architecture|
	...
	script |test Observer|
		...
		shouldRaise(cantGetVar, DoesNotInheritFromTopLevel, Â
...
script |assert instance of|
	...
	script |test classes of expressions|
		...
		assertInstanceOf(Çclass furlÈ, f) -- shouldn't be 'file'?
...
script |shouldRaise|
	...
	script |shouldRaise can catch more than one exception|
		...
		shouldRaise({}, Raiser, Â
			"The script should have raised exception 9876")
		shouldNotRaise({1, 2, 3, 1000, 10000}, Raiser, Â
...
script |pretty print|
	...
	script |pp alias|
		...
		assertEqual("alias" & space & ((path to me) as text), Â
	...
	script |pp application|
		property parent : UnitTest(me)
		assertEqual("Çapplication com.apple.finderÈ", pp(application "Finder"))
		assertEqual("Çapplication com.apple.finderÈ", Â
	...
	script |pp handler|
		...
		assertEqual("ÇhandlerÈ", pp(f))
		assertEqual("a reference to ÇhandlerÈ", pp(a reference to f))
	end script
	...
	script |pp list|
		...
		assertEqual("{Çscript pp listÈ, Çrecord {1, {Çapplication com.apple.finderÈ, {1, 2}}, x}È, true}", Â
	...
	script |pp POSIX file|
		...
		assertEqual("a reference to a reference to file" & space & (f as text), Â
			pp(f2))
	end script
	
	script |pp record|
		property parent : UnitTest(me)
		assertEqual("Çrecord {1, 2, 3}È", pp({a:1, b:2, c:3}))
	end script
	...
	script |pp script|
		...
		assertEqual("Çscript pp scriptÈ", pp(me))
		assertEqual("Çscript pp scriptÈ", pp(a reference to me))
		assertEqual("Çscript pp scriptÈ", pp(a reference to (a reference to me)))
		assertEqual("Çscript com.lifepillar.ppscriptÈ", pp(ppScript))
		assertEqual("a reference to Çscript com.lifepillar.ppscriptÈ", pp(ppScriptRef))
	end script
	...
	script |pp script called 'missing value'|
		...
		assertEqual("Çscript missing valueÈ", pp(ppScript))
		assertEqual("a reference to Çscript missing valueÈ", pp(ppScriptRef))
	end script
	...
	script |pp ASUnit|
		...
		assertEqual("a reference to Çscript" & space & ASUnit's id & "È", pp(TestASUnit's ASUnit))
		assertEqual("a reference to a reference to Çscript" & space & ASUnit's id & "È", pp(scriptRef))
	end script
	...
	script |pp object of class self|
		...
		assertEqual("Çobject of class selfÈ", pp(Self))
	end script
	...
	script |pp recursive undefined|
		...
		assertEqual("Çobject of class Çundefined referenceÈÈ", pp(SX))
		assertEqual("Çobject of class Çobject of class Çundefined referenceÈÈÈ", pp(SY))
	end script
	...
	script |pp recursive loop|
		...
		assertEqual("Çobject of class a reference to Çobject of class Çobject of class a reference to ...ÈÈÈ", pp(SX))
		assertEqual("Çobject of class Çobject of class a reference to Çobject of class Çobject of class ...ÈÈÈÈ", pp(SY))
	end script
