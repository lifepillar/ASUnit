		...
		on assertEqual(expected, value)
			...
			if value's class is not expected's class then
				try -- to pretty print the classes
					set msg to "Expected class: " & pp(expected's class) & linefeed & ¬
		...
		*)
		...
		on pp(anObject)
			if class of anObject is in {list, RGB color} then
				...
				return "«script " & anObject's name & "»"
			else if class of anObject is in {application, null} then
				return "«application " & anObject's name & "»"
			else
				set res to anObject as text
				if class of anObject is in {alias, boolean, class, constant, ¬
		...
		on loadTestsFromFolder(aFolder)
			...
			tell application "Finder"
				set testFiles to files of aFolder ¬
					where name starts with prefix and name ends with ¬
					".scpt" and name does not start with ¬
		...
		*)
		on compileSourceFiles(aFolder)
			tell application "Finder"
				set testFiles to files of aFolder ¬
					where name starts with prefix and name ends with ¬
					".applescript" and name does not start with ¬
					"Test Load" and name does not start with "TestLoad"
			end tell
			repeat with aFile in testFiles
				set outfile to (text 1 thru -(2 + (length of (aFile's name extension as text))) ¬
					of (aFile's name as text)) & ".scpt"
				set cmd to "osacompile -d -o " & space & ¬
					quoted form of (POSIX path of (aFolder as alias) & outfile) & space & ¬
